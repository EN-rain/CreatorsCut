...
public sealed class CacheService
{
    ...
    public async Task EvictOldEntries(CancellationToken ct)
    {
        ...
        await EnforceSizeLimit(Path.Combine(_cacheRoot, "thumbnails"), _policy.ThumbnailCacheSizeBytes, ct);
        await EnforceSizeLimit(Path.Combine(_cacheRoot, "waveforms"), _policy.WaveformCacheSizeBytes, ct);
        await EnforceSizeLimit(Path.Combine(_cacheRoot, "frames"), _policy.FrameCacheSizeBytes, ct);
        await EnforceSizeLimit(Path.Combine(_cacheRoot, "proxy"), _policy.ProxyCacheSizeBytes, ct);
    }
    ...
    private static Task EnforceSizeLimit(string directory, long limitBytes, CancellationToken ct)
    {
        if (!Directory.Exists(directory) || limitBytes <= 0) return Task.CompletedTask;
        ...
        {
            if (ct.IsCancellationRequested || total <= limitBytes) break;
